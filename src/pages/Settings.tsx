...
import { ProductUploadTestTool } from "@/components/settings/ProductUploadTestTool";
    ...
    <TabsTrigger value="facebook-comments-test" className="gap-2 flex-1 min-w-fit">
      ...
      Livestream Comment Test
        ...
        <TabsContent value="barcode" className="space-y-6 mt-4">
          {/* Product Upload Test Tool */}
          <ProductUploadTestTool />
        ...
        <TabsContent value="printer" className="space-y-6 mt-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Điều chỉnh DPI, Threshold và Width để tối ưu chất lượng in bill (sử dụng pdftoppm + sharp)
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* DPI Setting */}
              <div className="space-y-2">
                <label className="text-sm font-medium">DPI (Độ phân giải)</label>
                <div className="flex gap-2 items-center">
                  <Input
                    ...
                    max="600"
                    step="50"
                    defaultValue={localStorage.getItem('printDPI') || '300'}
                    onChange={(e) => localStorage.setItem('printDPI', e.target.value)}
                    className="w-32"
                  />
                  <Badge variant="outline">Mặc định: 300</Badge>
                </div>
                <p className="text-xs text-muted-foreground">
                  Cao hơn = chữ rõ hơn nhưng file lớn hơn (150-600)
                </p>
              </div>
              
              {/* Threshold Setting */}
              <div className="space-y-2">
                <label className="text-sm font-medium">Threshold (Ngưỡng đen/trắng)</label>
                <div className="flex gap-2 items-center">
                  <Input
                    type="number"
                    min="80"
                    max="150"
                    step="5"
                    defaultValue={localStorage.getItem('printThreshold') || '115'}
                    onChange={(e) => localStorage.setItem('printThreshold', e.target.value)}
                    className="w-32"
                  />
                  <Badge variant="outline">Mặc định: 115</Badge>
                </div>
                <p className="text-xs text-muted-foreground">
                  Thấp hơn = giữ nét mỏng, cao hơn = chữ đậm hơn (80-150)
                </p>
              </div>
              
              {/* Width Setting */}
              <div className="space-y-2">
                <label className="text-sm font-medium">Width (Chiều rộng in)</label>
                <div className="flex gap-2 items-center">
                  <Select
                    defaultValue={localStorage.getItem('printWidth') || '944'}
                    onValueChange={(value) => localStorage.setItem('printWidth', value)}
                  >
                    <SelectTrigger className="w-64">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="576">576px (80mm @ 203 DPI)</SelectItem>
                      <SelectItem value="800">800px (80mm @ 254 DPI)</SelectItem>
                      <SelectItem value="944">944px (80mm @ 300 DPI) ⭐</SelectItem>
                      <SelectItem value="1200">1200px (80mm @ 384 DPI)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <p className="text-xs text-muted-foreground">
                  Chiều rộng ảnh in (phụ thuộc DPI)
                </p>
              </div>
              ...
              <div className="pt-4 border-t">
                <Button
                  onClick={async () => {
                    const { getActivePrinter, printPDFToXC80 } = await import("@/lib/printer-utils");
                    const activePrinter = getActivePrinter();
                    
                    if (!activePrinter) {
                      toast({
                        title: "⚠️ Chưa có máy in",
                        description: "Vui lòng thêm và kích hoạt máy in trước",
                        variant: "destructive"
                      });
                      return;
                    }
                    ...
                    testPdf.text('TEST PRINT', 40, 15, { align: 'center' });
                    
                    testPdf.setFont('helvetica', 'normal');
                    testPdf.setFontSize(10);
                    testPdf.text('Nguyễn Văn Ánh', 40, 25, { align: 'center' });
                    testPdf.text('SĐT: 0901234567', 40, 32, { align: 'center' });
                    testPdf.text('Địa chỉ: 123 Đường ABC, Q1', 40, 39, { align: 'center' });
                    
                    const pdfDataUri = testPdf.output('datauristring');
                    ...
                    const result = await printPDFToXC80(activePrinter, pdfDataUri, {
                      dpi: printDPI,
                    ...
                    if (result.success) {
                      toast({
                        title: "✅ Test in thành công",
                        description: `DPI: ${printDPI}, Threshold: ${printThreshold}, Width: ${printWidth}px`
                      });
                    } else {
                      toast({
                        title: "❌ Test in thất bại",
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  ⚠️ Lưu ý: Bridge server phải cài đặt <code className="bg-muted px-1 py-0.5 rounded">pdftoppm</code> (poppler-utils)
                </p>
              </div>
            </CardContent>
          </Card>
