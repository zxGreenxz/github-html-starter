...
/**
 * Update product on TPOS using UpdateV2 endpoint
 * This endpoint accepts full product payload and updates existing product
 */
export async function updateProductOnTPOSV2(
  productPayload: any
): Promise<any> {
  const token = await getActiveTPOSToken();
  if (!token) throw new Error("TPOS Bearer Token not found");
  
  console.log(`📝 [TPOS UpdateV2] Updating product ${productPayload.DefaultCode}...`);
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.error(`❌ [TPOS UpdateV2] Failed:`, errorText);
    throw new Error(`UpdateV2 failed: ${response.status} - ${errorText.substring(0, 200)}`);
  }
  
  console.log(`✅ [TPOS UpdateV2] Product updated successfully`);
  
  // UpdateV2 có thể trả về 204 No Content hoặc JSON
 ...
 */
export async function deleteProductAndVariantsFromInventory(
  baseProductCode: string
): Promise<void> {
  console.log(`🗑️ [Inventory] Deleting ${baseProductCode} and variants...`);
...
/**
 * Fetch product from TPOS and upsert to local inventory
 * Includes parent product + all variants
 */
export async function fetchAndUpsertProductFromTPOS(
  ...
  // Fetch với expand ProductVariants và AttributeValues
  ...
  console.log(`✅ [TPOS] Fetched: ${tposProduct.Name}`);
  ...
  console.log(`✅ [Inventory] Upserted parent: ${baseProductCode}`);
