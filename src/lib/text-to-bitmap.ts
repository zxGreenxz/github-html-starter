 ...
 */
export async function textToBitmap(text: string, options: BitmapOptions = {}): Promise<BitmapResult> {
  const {
 ...
 */
export async function textToESCPOSBitmap(text: string, options: BitmapOptions = {}): Promise<Uint8Array> {
  ...
  const cutCommands = new Uint8Array([
    0x1b,
    0x64,
    0x03, // ESC d 3 - Feed 3 lines
    0x1d,
    0x56,
    0x42, // GS V 66 - Feed and full cut paper
  ]);
