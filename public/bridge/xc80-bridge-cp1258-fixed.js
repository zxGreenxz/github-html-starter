...
app.post('/print/bitmap', async (req, res) => {
  try {
    const { printerIp, printerPort, bitmapBase64, feeds = 3 } = req.body;

    if (!printerIp || !printerPort || !bitmapBase64) {
      return res.status(400).json({ 
        error: 'Missing required fields: printerIp, printerPort, bitmapBase64' 
      });
    }

    console.log(`📄 Printing ESC/POS bitmap to ${printerIp}:${printerPort}`);
    console.log(`Bitmap base64 length: ${bitmapBase64.length} characters`);

    // Decode base64 string to Uint8Array
    const binaryString = atob(bitmapBase64);
    const uint8Data = new Uint8Array(binaryString.length);
    for (let i = 0; i < binaryString.length; i++) {
      uint8Data[i] = binaryString.charCodeAt(i);
    }

    console.log(`Bitmap size: ${uint8Data.length} bytes`);
